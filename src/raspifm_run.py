...
if log:
    ...
    #gunicorn socket
    Popen(["/usr/bin/local/raspifm/.venv/bin/python3", "-m", "gunicorn", "--name", "raspifm web", "--timeout", "120", "--workers", "1", "--bind", "unix:/usr/bin/local/raspifm/raspifm.sock", "-m", "007", "webui_run:app"], cwd="/usr/bin/local/raspifm", stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    #touchui
